...
import { formatPrice, formatNumber, getListingReference, resolvePriceDisplay } from "~/lib/utils";
...
export const PropertyCard = React.memo(function PropertyCard({
  ...
  const hasBeds = !hideRooms && listing.bedrooms != null && listing.bedrooms > 0;
  const hasBaths =
    !hideRooms &&
    ...
    <>
      ...
      {hasArea && (
        <div className="flex items-center">
          ...
          <span>
            {formatNumber(listing.squareMeter ?? 0)}{" "}
            m²
          </span>
        </div>
      )}
