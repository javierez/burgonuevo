...
interface SortDropdownProps {
  slugString: string;
  currentSort?: string;
  /** Active free-text query, carried across so sorting doesn't drop it. */
  query?: string;
}

export function SortDropdown({ slugString, currentSort = "default", query }: SortDropdownProps) {
      ...
      <DropdownMenuContent>
        {sortOptions.map((option) => (
          <DropdownMenuItem key={option.value} asChild>
            <Link href={buildHref(option.value)} className="flex w-full items-center justify-between">
              {option.label}
